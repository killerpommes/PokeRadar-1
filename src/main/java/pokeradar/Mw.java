...
import pokeradar.gui.MwGui;
import pokeradar.gui.MwGuiMarkerDialog;
import pokeradar.gui.MwGuiMarkerDialogNew;
import pokeradar.map.*;
import pokeradar.overlay.OverlaySlime;
import pokeradar.region.BlockColours;
import pokeradar.region.RegionManager;
import pokeradar.tasks.CloseRegionManagerTask;
		...
		if ((this.mc.currentScreen == null) && (this.ready))
		{
			...
			else if (kb == MwKeyHandler.keyMapMode)
			{
				...
				if (newMap == null) {
					...
					//GlStateManager.clear(GlStateManager.resetColor(););
