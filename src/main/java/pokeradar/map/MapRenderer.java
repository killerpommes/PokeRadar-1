...
public class MapRenderer
{
	...
	public void drawEntityArrow(double x, double z, double r, boolean isPlayer) {
		...
		Point.Double p = this.mapMode.getClampedScreenXY(this.mapView, x * scale, z * scale);

		if (isPlayer) {
			Render.setColour(-16777216);
			Render.drawArrow(p.x, p.y, r, 6.5D);
			Render.setColour(-16776961);
			Render.drawArrow(p.x, p.y, r, 5D);
		} else {
	...
	private void drawRadarName()
	{
		if (this.mapMode.getConfig().getConfigCategory().equals(Reference.catSmallMapConfig))
		{
			GlStateManager.pushMatrix();
			GlStateManager.translate(this.textX, this.mapMode.getY() - textOffset - 5, 0);
			Render.drawCentredString(
					0,
					0,
					this.mapMode.getTextColour(),
