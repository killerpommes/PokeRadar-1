...
import pokeradar.entities.EntityFinder;
import pokeradar.map.PKMarker;
import org.lwjgl.input.Keyboard;
import org.lwjgl.input.Mouse;

import pokeradar.Mw;
import pokeradar.api.ILabelInfo;
import pokeradar.api.IMwDataProvider;
import pokeradar.api.MwAPI;
import pokeradar.config.Config;
import pokeradar.config.WorldConfig;
import pokeradar.forge.MwKeyHandler;
import pokeradar.map.MapRenderer;
import pokeradar.map.MapView;
import pokeradar.map.Marker;
import pokeradar.map.mapmode.MapMode;
import pokeradar.tasks.MergeTask;
import pokeradar.tasks.RebuildRegionsTask;
...
public class MwGui extends GuiScreen
{
	...
	public PKMarker getMarkerNearScreenPosPK(int x, int y)
	{
		...
		if (EntityFinder.pkMarkers != null) {
			for (PKMarker marker : EntityFinder.pkMarkers) {
				if ((marker != null) &&
						(marker.screenPos != null)) {
					if (marker.desc != null)
					{
						if (calculateInside(x, y, marker)) {
							nearMarker = marker;
						}
					}
					else if (marker.screenPos.distanceSq(x, y) < 9.0D) {
						nearMarker = marker;
					}
				}
			}
	...
	{
		if (Math.sqrt(Math.pow(m.screenPos.x - x, 2.0D) + Math.pow(m.screenPos.y - y, 2.0D)) <= m.r) {
			return true;
		}
		return false;
	}
				...
				this.markerLabel.setText(new String[]
						{
								pkMarker.name,
								pkMarker.desc,
								String.format("(%d, %d, %d)", pkMarker.coordX, pkMarker.coordY, pkMarker.coordZ)
						}, null);
			} else {
				this.markerLabel.setText(new String[]
				{
